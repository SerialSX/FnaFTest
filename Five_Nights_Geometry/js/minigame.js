    ...
    maoDoJogador.forEach(palavra => {
        ...
        span.onclick = () => {
            if (palavra === palavraInfectada) {
                ...
                console.log("Sistema restaurado. Vírus eliminado.");
            } else {
                // ERROU! Punição pesada!
                document.getElementById('terminal-overlay').classList.add('hidden');
                terminalOutput.innerHTML = '';

                // Dispara o alerta no topo da tela e quebra a câmera
                alert("FALHA CRÍTICA! Sintaxe válida detectada. O vírus destruiu um node de vídeo!");

                // Chama a função que criamos no game.js
                if (typeof breakNextCamera === "function") {
                    breakNextCamera();
                }
            }
        };
